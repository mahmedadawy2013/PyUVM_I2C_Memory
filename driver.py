    ...
    async def run_phase(self):
        while True:
            ...
            if (self.t_drive.rst == 1 ):
                self.dut_driver.rst.value     = 1 
                self.dut_driver.wr.value      = self.t_drive.wr
                self.dut_driver.addr.value    = self.t_drive.addr
                self.dut_driver.din.value     = self.t_drive.din
